// Math Magic Card Trick — математический фокус с картами
// Использует транспонирование матрицы (4x4) и перестановку столбцов
// Игрок загадывает число, программа "угадывает" его через два раунда

use rand::seq::SliceRandom;
use rand::thread_rng;
use std::io;

/// Выводит матрицу 4x4 в читаемом виде
...
fn main() {
    // 1) Генерируем перемешанную матрицу 4x4 с числами 1..16
    ...
    // 2) Игрок выбирает столбец с загаданным числом
    let col1: usize = loop {
    ...
    // 3) Транспонируем матрицу и переупорядочиваем столбцы
    // Математический трюк: нужный элемент всегда оказывается в первой строке
    ...
    // Переставляем столбцы: начиная с выбранного, затем остальные по порядку
    ...
    // 5) "Угадываем" карту — она всегда в первой строке выбранного столбца
    ...
    println!("3. Пересечение строки и столбца всегда даёт загаданное число");
}
