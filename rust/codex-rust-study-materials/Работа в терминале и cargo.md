...
Этот файл играет ту же роль, что и краткий терминальный гайд в исходном репозитории, но адаптирован под повседневную работу с Rust.
...
- Ошибка borrow checker не означает, что язык мешает. Обычно это сигнал, что надо пересмотреть владение данными и границы ответственности функций.
- Если функция не должна забирать владение, сначала подумайте о `&str`, `&[T]` или `&T`.
- Если вы пишете библиотечный код, `unwrap` и `expect` обычно плохой знак. Лучше вернуть `Result`.
