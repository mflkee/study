...
└── scripts/
    ├── modbus_client.py    # Тестовый клиент RTU (как ПЛК/SCADA)
    ├── modbus_tcp_client.py# Тестовый клиент Modbus TCP (stdlib, MBAP)
    ├── zynq_simulator.py   # Симулятор Zynq (опрос датчиков в реальном времени)
    └── simulator.py        # Симулятор полевых устройств
└── tools/
    └── portscheck/         # Быстрый raw-опрос ESP32: READ INPUT 60 рег. + hex
```
...
**Учебный режим прямо в TUI (без скриптов и без отключений):** вкладка
**Bus** показывает каждый Modbus-кадр как сырые байты RTU (`→` запрос,
`←` ответ) с CRC и временем — на эмуляторе и на реальной ESP32. Как читать
кадры и что будет на шине, когда мастер станет Zynq (RS-485/MAX485 +
настоящие датчики), разобрано в `tui/DOCUMENTATION.md`, разделы 14–15.

**Modbus TCP для будущего Zynq по Ethernet:** вкладка Ports → `[t]` (или
Help в TUI) поднимает TCP-сервер на `127.0.0.1:1502`, отдающий ту же карту
регистров, что и эмулятор/ESP32. Попробовать:

```bash
python3 scripts/modbus_tcp_client.py 127.0.0.1 1502   # подробности: tui/DOCUMENTATION.md, раздел 16
```
